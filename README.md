...
| **API 版本协商** | SDK 声明版本 `2.1.2`；mod 声明 `SdkVersion`，要求高于当前的 mod 被拒绝加载。`doorstop_config.json` 的 `sdkVersion` 声明当前版本 |
...
```json
{
  ...
  "sdkVersion": "2.1.2"               // 当前 SDK 版本(校验 mod 的 SdkVersion)
}
```
