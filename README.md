...
| **API 版本协商** | SDK 声明版本 `2.1.7`；mod 声明 `SdkVersion`，要求高于当前的 mod 被拒绝加载。`doorstop_config.json` 的 `sdkVersion` 声明当前版本 |
...
```
git tag modloader-2.1.7
git push origin modloader-2.1.7
```
...
产物 (Release 资产):
- `cesium-loader-2.1.7.zip` — 加载器部署包 (带版本号)
- `cesium-loader.zip` — 固定名, 供 `releases/latest/download/cesium-loader.zip` 使用
- `cesium-sdk-tools-2.1.7.zip` — SDK 工具包 (带版本号)
